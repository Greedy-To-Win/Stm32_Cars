...
//开时钟	 
	RCC_APB2PeriphClockCmd(DeBug_Rcc_GPIOx, ENABLE);	  //使能UGPIOA时钟
	RCC_APB1PeriphClockCmd(DeBug_Rcc_USARTx, ENABLE);     //使能USART1时钟
//USART1_TX  
	GPIO_InitStructure.GPIO_Pin = DeBug_TX; 			  //PB10
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;		  //复用推挽输出
	GPIO_Init(DeBug_Port, &GPIO_InitStructure);
   
//USART1_RX	  
	GPIO_InitStructure.GPIO_Pin = DeBug_RX;			 	 //PA11
...
void DeBug_IRQHandler(void)                	//串口1中断服务程序
	{
	u8 Res;

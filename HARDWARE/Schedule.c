...
void TIM1_Config(u16 arr,u16 psc)
{
    ...
    TIM_ClearITPendingBit(TIM6,TIM_IT_Update);TIM_ClearFlag(TIM1,TIM_FLAG_Update);//??????
    TIM_TimeBaseStructure.TIM_Period =    arr;
    TIM_TimeBaseStructure.TIM_Prescaler = psc;
...
/**************电机速度外环**************/
		...
		{
			//外环
			Motor_Velocity_Set(40);//期望速度
			//电机速度环控制，最大输出为3999
			Inc_PID_Control(&Motor_Velocity_loop,3999);
...
void TIM1_IRQHandler(void)//1ms//1000HZ
{

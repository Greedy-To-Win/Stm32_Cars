...
{ 
	...
	OLED_ShowString(1,1,"MPU_Init");
	MPU_Write_Byte(MPU_PWR_MGMT1_REG,0X80);	//复位MPU6050
    delay_ms(100);
	MPU_Write_Byte(MPU_PWR_MGMT1_REG,0X00);	//唤醒MPU6050 
	MPU_Set_Gyro_Fsr(3);					//陀螺仪传感器,±2000dps
	MPU_Set_Accel_Fsr(0);					//加速度传感器,±2g
	MPU_Set_Rate(50*4);						//设置采样率200Hz 200*5=1000,5ms
	MPU_Write_Byte(MPU_INT_EN_REG,0X00);	//关闭所有中断
	MPU_Write_Byte(MPU_USER_CTRL_REG,0X00);	//I2C主模式关闭
	MPU_Write_Byte(MPU_FIFO_EN_REG,0X00);	//关闭FIFO
	MPU_Write_Byte(MPU_INTBP_CFG_REG,0X80);	//INT引脚低电平有效
	res=MPU_Read_Byte(MPU_DEVICE_ID_REG); 
//			sprintf(oledBuf,"res = %x",res);
...
float Scale_Transform(float Sample_Value, float URV, float LRV)
{
  ...
  * 返 回 值：无
  */
void MPU6050_GetAccel_Value(short *ax,short *ay,short *az)
{
    ...
    /*开始转换加速度值*/
    *ax = Scale_Transform( (float)ACCEL_Array[0], 16.0, -16.0);  //转换X轴
    *ay = Scale_Transform( (float)ACCEL_Array[1], 16.0, -16.0);  //转换Y轴
    *az= Scale_Transform( (float)ACCEL_Array[2], 16.0, -16.0);  //转换Z轴
}
//IIC连续写
//addr:器件地址 
//reg:寄存器地址
